...
//---Ranged Selector---------------------------------------------
class RangedSelector : public KnobParentClass
{
public:
    RangedSelector( void );
    ~RangedSelector( void );
    ...
    virtual void freshen( uint16_t msTickDelta );
    uint16_t getState( void );
    bool serviceChanged( void );
    float getStateRanged( int16_t low, int16_t high );
    bool serviceChangedAnalog( void );
...
private:
    ...
    //For sub-analog
    int16_t analogState;
    int16_t analogLastState;
    uint8_t hysteresis = 10;
    int8_t lastSlope = 1;
    bool analogNewData = false;
};
